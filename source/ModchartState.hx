...
class ModchartState 
{
	...
	{
		#if sys
		var data:BitmapData = BitmapData.fromFile(Sys.getCwd() + "assets/data/" + StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() + '/' + spritePath + ".png");

		var sprite:FlxSprite = new FlxSprite(0,0);

		sprite.frames = FlxAtlasFrames.fromSparrow(FlxGraphic.fromBitmapData(data), Sys.getCwd() + "assets/data/" + StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() + "/" + spritePath + ".xml");
    ...
    function new()
    {
				...
				var result = LuaL.dofile(lua, Paths.lua(StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() + "/modchart")); // execute le file
				...
				Lua_helper.add_callback(lua,"doubleUghLmao", function(id:String,anim:String,force:Bool = false,reverse:Bool = false) {
					getActorByName(id).playAnim(anim, force, reverse);
					var timer:FlxTimer = new FlxTimer();
					timer.start(0.1, function (timelol:FlxTimer) {
						getActorByName(id).playAnim(anim, force, reverse);
					});
				});
